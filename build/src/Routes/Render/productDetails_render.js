...
productDetails_route.post('/target', tokenValidate_1.default, function (req, res) {
    ...
    res.json({ name: name, catagory: catagory, description: description, image: image });
});
productDetails_route.get('/productDetails', tokenValidate_1.default, function (req, res) {
    try {
        console.log(image);
        res.render('product-details', { name: name, catagory: catagory, description: description, image: image });
    }
    catch (error) {
